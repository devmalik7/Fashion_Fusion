...
const PrimaryDropDownMenu = ({ setTogglePrimaryDropDown, user }) => {
    ...
    const navs = [
        ...
        // {
        //     title: "Fashion Fusion Hub Plus Zone",
    ...
    return (
        <div className="absolute w-60 -left-24 ml-2 top-9 bg-gray-300 shadow-2xl rounded flex-col text-sm">

            {user.role === "admin" &&
                <Link className="pl-3 py-3.5 border-b flex gap-3 items-center hover:bg-gray-50 rounded-t" to="/admin/dashboard">
                    <span className="text-gray-600"><DashboardIcon sx={{ fontSize: "18px" }} /></span>
                    Admin Dashboard
                </Link>
            }

            <Link className="pl-3 py-3.5 border-b flex gap-3 items-center hover:bg-gray-50 rounded-t" to="/account">
                <span className="text-gray-600"><AccountCircleIcon sx={{ fontSize: "18px" }} /></span>
                My Profile
            </Link>
            ...
            <div className="pl-3 py-3.5 flex gap-3 items-center hover:bg-gray-50 rounded-b cursor-pointer" onClick={handleLogout} >
                <span className="text-gray-600"><PowerSettingsNewIcon sx={{ fontSize: "18px" }} /></span>
                Logout
            </div>
