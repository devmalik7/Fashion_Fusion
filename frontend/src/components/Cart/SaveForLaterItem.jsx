...
const SaveForLaterItem = ({ product, name, seller, price, cuttedPrice, image, stock, quantity }) => {
    ...
    return (
        <div className="flex flex-col gap-3 bg-gray-300 py-5 pl-2 sm:pl-6 border-b" key={product}>
            ...
            <div className="flex justify-evenly sm:justify-start sm:gap-6">
                ...
                {/* <!-- quantity --> */}
                <button onClick={() => moveToCartHandler(product, quantity)} className="sm:ml-4 font-medium hover:text-gray-600 ">MOVE TO CART</button>
                <button onClick={() => removeFromSaveForLaterHandler(product)} className="font-medium hover:text-pink-400">REMOVE</button>
            </div>
            {/* <!-- move to cart --> */}
