...
const PriceSidebar = ({ cartItems }) => {
    return (
        <div className="flex sticky top-16 sm:h-screen bg-gray-300 flex-col sm:w-4/12 sm:px-1">

            {/* <!-- nav tiles --> */}
            <div className="flex flex-col bg-grayy-300 rounded-sm shadow">
                ...
                <div className="flex flex-col gap-4 p-6 pb-3">
                    ...
                    <p className="flex justify-between">Discount <span className="text-pink-400">- ₹{cartItems.reduce((sum, item) => sum + ((item.cuttedPrice * item.quantity) - (item.price * item.quantity)), 0).toLocaleString()}</span></p>
                    <p className="flex justify-between">Delivery Charges <span className="text-pink-400">FREE</span></p>
