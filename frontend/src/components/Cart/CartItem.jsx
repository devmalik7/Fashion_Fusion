...
const CartItem = ({ product, name, seller, price, cuttedPrice, image, stock, quantity, inCart }) => {
    ...
    return (
        <div className="flex flex-col gap-3 py-5 pl-2 bg-gray-300 sm:pl-6 border-b overflow-hidden" key={product}>
                ...
                <div className="flex flex-col sm:gap-5 w-full pr-6">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start pr-5 gap-1 sm:gap-0">
                        <div className="flex flex-col gap-0.5 sm:w-3/5">
                            <p className="group-hover:text-pink-300">{name.length > 42 ? `${name.substring(0, 42)}...` : name}</p>
                    ...
                    <div className="flex items-baseline gap-2 text-xl font-medium">
                        ...
                        <span className="text-sm text-gray-600">{getDiscount(price, cuttedPrice)}%&nbsp;off</span>
                    </div>
                    {/* <!-- price desc --> */}
            ...
            <div className="flex justify-between pr-4 sm:pr-0 sm:justify-start sm:gap-6">
                ...
                <div className="flex gap-1 items-center">
                    <span onClick={() => decreaseQuantity(product, quantity)} className="w-7 h-7 text-3xl font-light bg-gray-300 rounded-full border flex items-center justify-center cursor-pointer"><p>-</p></span>
                    ...
                    <span onClick={() => increaseQuantity(product, quantity, stock)} className="w-7 h-7 text-xl font-light bg-gray-300 rounded-full border flex items-center justify-center cursor-pointer">+</span>
                </div>
                {/* <!-- quantity --> */}
                {inCart && (
                    <>
                    <button onClick={() => saveForLaterHandler(product)} className="sm:ml-4 font-medium bg-gray-300 hover:text-gray-600">SAVE FOR LATER</button>
                    <button onClick={() => removeCartItem(product)} className="font-medium hover:text-pink-400">REMOVE</button>
                    </>
                )}
            </div>
            {/* <!-- save for later --> */}
